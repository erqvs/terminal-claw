...
const BRIDGE_URL = process.env.OPENCLAW_CRON_BRIDGE_URL || 'http://172.18.0.1:3011';
const BRIDGE_TOKEN = security_1.OPENCLAW_CRON_BRIDGE_TOKEN || '';
...
router.post('/jobs', async (req, res) => {
    try {
        const requestBody = normalizeScheduleBody(req.body);
...
router.post('/jobs/:id/enable', async (req, res) => {
    const id = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
    try {
...
router.post('/jobs/:id/disable', async (req, res) => {
    const id = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
    try {
